...
def split_by_seconds(filename, split_length, output_dir="", vcodec="copy", acodec="copy",
    ...
    durations_str = [i for i in range(0, video_length, split_length)]
    durations_str.append(video_length)

    if os.path.exists(output_dir) and os.path.isdir(output_dir):
        output_file = os.path.join(output_dir, os.path.basename(filebase))
    else:
    ...
    for n in range(0, split_count):
        ...
        split_args += ["-ss", str(split_start), "-t", str(split_length),
                       output_file + "_" + str(durations_str[n]) + "_" +
                       str(durations_str[n + 1]) + "." + fileext]
...
def main():
    ...
    parser.add_option("-o", "--output_dir",
                      dest="output_dir",
                      help="Path to output directory to save the splitted videos.",
                      type="string",
                      default="",
                      action="store"
                      )
