...
resource "aws_lambda_function" "s3-presigned-url-python" {
  function_name    = "s3-presigned-url-python"
  filename         = "${path.cwd}/modules/ingress/lambda_s3_presigned_url.py.zip"
  handler          = "lambda_s3_presigned_url.lambda_handler"
  source_code_hash = filebase64sha256("${path.cwd}/modules/ingress/lambda_s3_presigned_url.py.zip")
...
resource "aws_lambda_permission" "apigw-permission" {
  ...
  function_name = aws_lambda_function.s3-presigned-url-python.function_name
