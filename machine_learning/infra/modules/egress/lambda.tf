...
resource "aws_lambda_function" "s3-egress-presigned-url-python" {
  ...
  filename         = "${path.cwd}/modules/egress/lambda_s3_egress_presigned_url.py.zip"
  handler          = "lambda_s3_egress_presigned_url.lambda_handler"
  source_code_hash = filebase64sha256("${path.cwd}/modules/egress/lambda_s3_egress_presigned_url.py.zip")
